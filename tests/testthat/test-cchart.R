...
test_that("flipStandardCharts::Chart chart functions",{
    ...
    expect_warning(print(CChart("Column", pie.sales, sfdsmain = "My pie sales", warn.if.no.match = FALSE)), "bargroupgap")
...
#                             main = "My pie sales", warn.if.no.match = FALSE)), NA)
    ...
    CChart("Bar", cbind(X=1:10, Y=1:10), values.tick.format=".2f", type = "Stacked")
