                           ...
                           table.counter = 1)
{
    ...
    if (isTRUE(transpose))
    {
        if (length(dim(data)) > 2 || !inherits(data, "QTable"))
        {
            # Need to manually handle attributes
            ...
            data <- t(data)         # But usually attributes are handled by t.QTable in the verbs package
    }
